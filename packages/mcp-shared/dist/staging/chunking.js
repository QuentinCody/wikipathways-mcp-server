...
export class ChunkingEngine {
    CHUNK_SIZE_THRESHOLD = 32 * 1024; // 32KB
    ...
    async smartJsonParse(value, sql) {
        ...
        if (content == null)
            throw new Error(`Missing chunked content ${id}`);
    ...
    ensureTables(sql) {
		...
		`);
        sql.exec(`CREATE INDEX IF NOT EXISTS idx_chunks_lookup ON content_chunks(content_id, chunk_index);`);
    }
    async storeChunkedContent(content, contentType, sql) {
        ...
        for (let i = 0; i < parts.length; i++) {
            sql.exec(`INSERT INTO content_chunks (content_id, chunk_index, chunk_data, chunk_size) VALUES (?, ?, ?, ?)`, contentId, i, parts[i], parts[i].length);
        }
        const meta = {
            contentId,
        ...
        sql.exec(`INSERT INTO chunk_metadata (content_id, total_chunks, original_size, content_type, compressed) VALUES (?, ?, ?, ?, 0)`, meta.contentId, meta.totalChunks, meta.originalSize, meta.contentType);
        return meta;
    }
    async retrieveChunkedContent(contentId, sql) {
        const rows = sql
            .exec(`SELECT chunk_data FROM content_chunks WHERE content_id = ? ORDER BY chunk_index ASC`, contentId)
            .toArray();
