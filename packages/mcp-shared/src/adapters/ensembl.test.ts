...
describe("ensembl adapter", () => {
	it("ensemblGet resolves JSON from GRCh38 by default", async () => {
		...
		const f: typeof fetch = async (input) => {
			...
			return new Response(JSON.stringify({ id: "rs7903146", mappings: [] }), {
				status: 200,
			});
		};
		const out = await ensemblGet<{ id: string }>("/variation/human/rs7903146", {
			fetchImpl: f,
		});
	...
	it("ensemblPost sends JSON body", async () => {
		const out = await ensemblPost<unknown>(
			"/vep/human/region",
			{ variants: ["1 100 . A G"] },
			{ fetchImpl: stub({ ok: true }) },
		);
