...
function mockApiFetch(pool: number) {
	...
	const apiFetch: ApiFetchFn = async (request) => {
		...
		const all = Array.from({ length: pool }, (_, i) => ({ id: i, name: `item-${i}` }));
		return { status: 200, data: { count: pool, results: all.slice(offset, offset + limit) } };
	};
	...
	return {
		...
		get: () => ({
			async fetch(req: Request) {
				...
				if (path === "/process")
					return json({ success: true, tables_created: ["t1"], total_rows: 2000, input_rows: 2000, table_row_counts: { t1: 2000 } });
...
describe("createPaginateProxyTool", () => {
	it("returns combined items inline with a completeness verdict", async () => {
		...
		const res = (await tool.handler({ path: "/search", params: { db: "x" }, opts: { pageSize: 5 } }, ctx)) as Record<string, unknown>;
		expect(res).toMatchObject({ count: 12, pages: 3, total_available: 12, completeness: { complete: true } });
		expect(calls[0]).toMatchObject({ method: "GET", path: "/search", params: { db: "x", offset: 0, limit: 5 } });
	});

	it("interpolates path params once and paginates the rest", async () => {
		const { apiFetch, calls } = mockApiFetch(3);
		const tool = createPaginateProxyTool({ apiFetch });
		await tool.handler({ path: "/gene/{id}/variants", params: { id: "BRCA1" }, opts: { pageSize: 10 } }, ctx);
	...
	it("auto-stages large combined sets when a DO namespace is configured", async () => {
		...
		const tool = createPaginateProxyTool({ apiFetch, doNamespace: makeStagingDo(), stagingPrefix: "entrez" });
		const res = (await tool.handler({ path: "/search", opts: { pageSize: 500 } }, ctx)) as Record<string, unknown>;
		...
		const wsNs = {
			...
			get: () => ({
				async fetch(req: Request) {
					const path = new URL(req.url).pathname;
					wsCalls.push(path);
					if (path === "/ws/stage") return json({ success: true, data_access_id: "ws_p_1", tables: ["entrez__t1"], row_count: 2000 });
					return json({ success: false }, 404);
				},
			}),
		} as never;
		const tool = createPaginateProxyTool({ apiFetch, doNamespace: makeStagingDo(), stagingPrefix: "entrez", workspaceNamespace: wsNs });
		const res = (await tool.handler({ path: "/search", opts: { pageSize: 500 } }, { sql: () => [], workspace: "W" })) as Record<string, unknown>;
		expect(res.__staged).toBe(true);
	...
	it("returns __api_error on fetch failure", async () => {
		...
		const tool = createPaginateProxyTool({ apiFetch });
		const res = (await tool.handler({ path: "/search" }, ctx)) as Record<string, unknown>;
	...
	it("rejects dangerous traversal paths", async () => {
		...
		const tool = createPaginateProxyTool({ apiFetch });
		const res = (await tool.handler({ path: "/../etc/passwd" }, ctx)) as Record<string, unknown>;
		expect(res.__api_error).toBe(true);
	});
});
