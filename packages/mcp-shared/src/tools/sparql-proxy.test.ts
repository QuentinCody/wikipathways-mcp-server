...
const selectEnvelope = {
	...
	results: {
		bindings: [
			...
			{ s: { value: "urn:c" }, p: { type: "bnode" } as unknown as { value: string } }, // present key, no .value → null
		],
	},
};
...
describe("buildStagedTableSummary", () => {
	const base = { dataAccessId: "d", schema: {}, tablesCreated: [] as string[], totalRows: 0 } as never;
	it("summarizes zero / one / many tables, with and without row counts", () => {
		expect(buildStagedTableSummary({ ...(base as object), tablesCreated: [], totalRows: 5 } as never)).toBe("5 rows");
		expect(buildStagedTableSummary({ ...(base as object), tablesCreated: ["t"], totalRows: 3 } as never)).toBe(
			'table "t" [3 rows]',
		);
		expect(
			buildStagedTableSummary({
	...
	it("falls back to 0 when row counts and totalRows are absent", () => {
		expect(buildStagedTableSummary({ tablesCreated: [] } as never)).toBe("0 rows");
		expect(buildStagedTableSummary({ tablesCreated: ["t"] } as never)).toBe('table "t" [0 rows]');
	});
});
...
describe("createSparqlProxyTool handler", () => {
	const tool = (over: Partial<Parameters<typeof createSparqlProxyTool>[0]> = {}) =>
		createSparqlProxyTool({ sparqlFetch: vi.fn(), stagingPrefix: "sparql", ...over });
	const ctx = { sessionId: "sess" } as ToolContext;

	it("rejects an empty query", async () => {
		const t = tool();
		expect(await t.handler({ query: "" } as never, ctx)).toMatchObject({ code: "invalid_input" });
	});
	...
	it("returns the raw envelope when staging is not triggered", async () => {
		...
		const result = await t.handler({ query: "SELECT * WHERE { ?s ?p ?o }" } as never, ctx);
	...
	it("auto-stages a large response and wraps it, preserving scalars", async () => {
		...
		const t = tool({ sparqlFetch: vi.fn(async () => envelope), doNamespace: {} });
		const result = (await t.handler({ query: "SELECT *", method: "GET", timeoutMs: 5 } as never, ctx)) as Record<string, unknown>;
		expect(result).toMatchObject({ __staged: true, data_access_id: "da-1", total_rows: 2, queryTimeMs: 12 });
	...
	it("maps Error and non-Error fetch failures to an execution error", async () => {
		const t = tool({ sparqlFetch: vi.fn(async () => { throw new Error("upstream 500"); }) });
		...
		const t2 = tool({ sparqlFetch: vi.fn(async () => { throw "raw failure"; }) });
		expect(await t2.handler({ query: "SELECT *" } as never, ctx)).toMatchObject({ message: "raw failure" });
	});
});
