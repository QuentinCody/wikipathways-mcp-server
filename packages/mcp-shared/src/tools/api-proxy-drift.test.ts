...
const catalog: ApiCatalog = {
	...
	endpointCount: 2,
	endpoints: [
		{ method: "GET", path: "/studies/{id}", summary: "Get a study", pathParams: [{ name: "id" }], queryParams: [] },
		{ method: "GET", path: "/studies", summary: "List studies", pathParams: [], queryParams: [{ name: "q" }] },
	],
...
describe("buildKnownEndpointIndex", () => {
	...
	it("merges catalog + OpenAPI spec endpoints", () => {
		const spec = {
			...
			paths: {
				"/genes/{gid}": {
					get: { summary: "gene", parameters: [{ in: "path", name: "gid" }, { in: "query", name: "expand" }] },
				},
			},
