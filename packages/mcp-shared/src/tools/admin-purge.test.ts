...
import { adminPurgeMixin, handleAdminPurge, type AdminPurgeResult } from "./admin-purge";

const OID = "a".repeat(64);
const TOKEN = "secret-token";

const makeStub = (over: Partial<Record<"__admin_purge__" | "__admin_stats__", unknown>> = {}) => ({
	__admin_purge__: vi.fn(async () => ({ purged: true, sqliteSizeBeforeBytes: 10, sqliteSizeAfterBytes: 0 })),
...
const adminRequest = (op: "purge" | "stats", oid = OID, init: RequestInit = {}) =>
...
describe("handleAdminPurge routing & auth", () => {
	it("returns null for non-admin paths", async () => {
		expect(await call(new Request("https://worker.test/mcp"), makeEnv())).toBeNull();
	});
	...
	it("returns 503 when ADMIN_TOKEN is not configured", async () => {
		const resp = await call(adminRequest("purge"), makeEnv(makeStub(), { ADMIN_TOKEN: undefined }));
		expect(resp?.status).toBe(503);
	});

	it("returns 403 on a token mismatch", async () => {
		const resp = await call(
			adminRequest("purge", OID, { method: "DELETE", headers: { "x-admin-token": "wrong" } }),
			makeEnv(),
		);
	...
	it("returns 500 when the DO binding is missing", async () => {
		const resp = await call(adminRequest("purge"), makeEnv(makeStub(), { MCP_OBJECT: undefined }));
	...
	it("returns 400 when the id cannot be resolved", async () => {
		const env = makeEnv();
		(env.MCP_OBJECT as { idFromString: ReturnType<typeof vi.fn> }).idFromString.mockImplementation(() => {
			throw new Error("bad id");
		});
		const resp = await call(adminRequest("purge"), env);
		expect(resp?.status).toBe(400);
		expect(await resp?.json()).toMatchObject({ error: "could not resolve id", message: "bad id" });
	});

	it("enforces DELETE for purge and GET for stats", async () => {
		const purgeViaGet = await call(adminRequest("purge", OID, { method: "GET" }), makeEnv());
		expect(purgeViaGet?.status).toBe(405);
		expect(await purgeViaGet?.json()).toMatchObject({ expected: "DELETE" });

		const statsViaDelete = await call(adminRequest("stats", OID, { method: "DELETE" }), makeEnv());
...
describe("handleAdminPurge RPC dispatch", () => {
	it("dispatches purge to __admin_purge__ and returns its result", async () => {
		...
		expect(await resp?.json()).toMatchObject({ purged: true, sqliteSizeAfterBytes: 0 });
	...
	it("wraps RPC failures as a 500 with op context", async () => {
		...
		const resp = await call(adminRequest("purge"), makeEnv(stub));
		expect(resp?.status).toBe(500);
		expect(await resp?.json()).toMatchObject({ error: "admin rpc failed", op: "purge", message: "rpc exploded" });
	});
});
...
describe("adminPurgeMixin", () => {
	it("purge reports sizes before and after deleteAll", async () => {
		...
		expect(result).toEqual({ purged: true, sqliteSizeBeforeBytes: 4096, sqliteSizeAfterBytes: 0 });
	...
	it("purge reports null sizes when databaseSize is unavailable or throws", async () => {
		const noSize = { deleteAll: async () => {}, sql: { exec: () => ({ toArray: () => [] }) } };
		...
		expect(await adminPurgeMixin.purge(throwing)).toMatchObject({ sqliteSizeBeforeBytes: null });
	});
	...
	it("stats lists tables and tolerates SQL failures", () => {
		...
		expect(adminPurgeMixin.stats(storage)).toEqual({ sqliteSizeBytes: 123, tables: ["alpha", "beta"] });
		...
		expect(adminPurgeMixin.stats(broken)).toEqual({ sqliteSizeBytes: 5, tables: [] });
	});
});
