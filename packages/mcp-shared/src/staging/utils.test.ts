...
describe("stageToDoAndRespond", () => {
	...
	it("flags materialization loss when the DO skips rows", async () => {
		const process = () =>
			json({
				...
				staging_warnings: {
					rows_skipped: 12,
					data_loss_warning: "12 of 100 rows (12.0%) failed to stage.",
				},
			});
		...
		expect(result._staging.completeness?.truncation?.detail).toContain(
			"12 of 100",
		);
	});
	...
	it("routes to the WorkspaceDO when options.workspace is present (skips per-server /process)", async () => {
		...
		const { ns: perServerNs, calls: perServerCalls } = makeDo({
			"/process": okProcess,
			"/schema": okSchema,
			"/register": () => json({ success: true }),
		});
...
describe("queryDataFromDo SQL guards", () => {
	...
	it("does not flag column names that merely contain keywords", async () => {
		const { ns } = makeDo({
			"/schema": okSchema,
			"/query": () =>
				json({ success: true, results: [{ created_at: "x" }], row_count: 1 }),
		});
		await expect(
			queryDataFromDo(ns, "civic_1", "SELECT created_at, updated_at FROM t"),
...
describe("queryDataFromDo execution", () => {
	...
	it("keeps an existing LIMIT", async () => {
		const { ns } = makeDo({
			"/schema": okSchema,
			"/query": () => json({ success: true, results: [] }),
		});
	...
	it("rejects an unknown/empty data_access_id (no user tables) with a 404", async () => {
		const { ns } = makeDo({
			"/schema": () =>
				json({
					success: true,
					schema: { tables: { _staging_x: {}, sqlite_seq: {} } },
				}),
		});
		await expect(
			queryDataFromDo(ns, "civic_missing", "SELECT 1"),
		).rejects.toThrow(/Unknown or empty data_access_id/);
	});
	...
	it("continues past a transient (non-404) probe failure", async () => {
		...
		await expect(
			queryDataFromDo(ns, "civic_1", "SELECT 1"),
...
describe("getSchemaFromDo", () => {
	...
	it("throws on failure, missing schema, or empty tables", async () => {
		await expect(
			getSchemaFromDo(
				makeDo({ "/schema": () => json({ success: false, error: "nope" }) }).ns,
				"x",
			),
		).rejects.toThrow(/nope/);
		await expect(
			getSchemaFromDo(
				makeDo({ "/schema": () => json({ success: true, schema: {} }) }).ns,
				"x",
			),
		).rejects.toThrow(/not found or contains no data/);
		await expect(
			getSchemaFromDo(
				makeDo({
					"/schema": () => json({ success: true, schema: { tables: {} } }),
				}).ns,
				"x",
			),
...
describe("createQueryDataHandler", () => {
	...
	it("returns a success response with meta on a good query", async () => {
		...
		const res = await handler(
			{ data_access_id: "civic_1", sql: "SELECT * FROM t", limit: 10 },
			{ DATA_DO: ns },
		);
	...
	it("maps error messages to codes", async () => {
		...
		const notFoundNs = makeDo({
			"/schema": () => json({ success: true, schema: { tables: {} } }),
		}).ns;
		const notFound = await handler(
			{ data_access_id: "civic_x", sql: "SELECT 1" },
			{ DATA_DO: notFoundNs },
		);
		expect(
			(notFound.structuredContent as { error: { code: string } }).error.code,
...
describe("createGetSchemaHandler", () => {
	...
	it("returns a specific dataset's schema, or an error", async () => {
		...
		const bad = await handler(
			...
			{
				DATA_DO: makeDo({
					"/schema": () => json({ success: false, error: "x" }),
				}).ns,
			},
		);
