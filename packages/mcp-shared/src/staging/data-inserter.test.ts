...
import { insertData, mapEntityToSchema, trackEntityRelationships } from "./data-inserter";
...
const noopSql = (): SqlExec =>
	({ exec: () => ({ toArray: () => [], one: () => undefined }) }) as unknown as SqlExec;
...
const mapTo = (obj: unknown, columns: Record<string, string>, state = makeState()) =>
	mapEntityToSchema(obj, schemaOf(columns), state);

describe("insertData", () => {
	it("inserts a scalar fallback row and reports no errors", () => {
		const schemas: Record<string, TableSchema> = { scalar_data: schemaOf({ value: "TEXT" }) };
...
describe("mapEntityToSchema", () => {
	...
	it("maps a direct column match", () => {
		expect(mapTo({ name: "Alice" }, { name: "TEXT" })).toEqual({ name: "Alice" });
	});

	it("coerces booleans to 0/1", () => {
		expect(mapTo({ active: true }, { active: "INTEGER" })).toEqual({ active: 1 });
		expect(mapTo({ active: false }, { active: "INTEGER" })).toEqual({ active: 0 });
	});

	it("serializes a nested object into a _json column", () => {
		expect(mapTo({ meta: { a: 1 } }, { meta_json: "TEXT" })).toEqual({ meta_json: '{"a":1}' });
	});

	it("skips an AUTOINCREMENT id column", () => {
		expect(mapTo({ id: 9, name: "x" }, { id: "INTEGER PRIMARY KEY AUTOINCREMENT", name: "TEXT" })).toEqual({
			name: "x",
		});
	});

	it("resolves a foreign key from a nested entity's own id", () => {
		expect(mapTo({ author: { id: 42, name: "A" } }, { author_id: "INTEGER" })).toEqual({ author_id: 42 });
	});
	...
	it("resolves a foreign key from the processed-entity registry when present", () => {
		...
		expect(mapEntityToSchema({ author }, schemaOf({ author_id: "INTEGER" }), state)).toEqual({
			author_id: 777,
		});
	});

	it("extracts a nested field via prefix_subfield", () => {
		expect(mapTo({ author: { name: "Carol" } }, { author_name: "TEXT" })).toEqual({ author_name: "Carol" });
	});
...
describe("trackEntityRelationships", () => {
	...
	it("records a junction pair for each related entity in an array", () => {
		...
		trackEntityRelationships({ tags: [tag1, tag2] }, "article", 10, schemas, state);
	...
	it("ignores non-entity items", () => {
		const state = makeState();
		trackEntityRelationships({ tags: ["plain", "strings"] }, "article", 10, schemas, state);
		expect(state.relationshipData.size).toBe(0);
	});
});
