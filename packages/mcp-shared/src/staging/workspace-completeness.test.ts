...
describe("workspaceCompleteness", () => {
	...
	it("flags incomplete materialization when the DO reports failed rows (no upstream total)", () => {
		expect(
			workspaceCompleteness(undefined, {
				row_count: 9,
				completeness: { complete: false, failed_rows: 3 },
			}),
	...
	it("reports incomplete materialization even when row_count is absent", () => {
		expect(
			workspaceCompleteness(undefined, { completeness: { complete: false } }),
	...
	it("reports complete when every upstream row was fetched", () => {
		expect(workspaceCompleteness(100, { row_count: 100 })).toEqual({
			complete: true,
		});
	});
	...
	it("flags INCOMPLETE when primary_row_count < upstreamTotal even though total row_count >= upstreamTotal", () => {
		...
		expect(
			workspaceCompleteness(50, { row_count: 105, primary_row_count: 5 }),
		).toEqual({
			complete: false,
			total_available: 50,
			returned: 105,
	...
	it("uses primary_row_count (not inflated total row_count) as the pagination denominator", () => {
		...
		expect(
			workspaceCompleteness(10, { row_count: 30, primary_row_count: 3 }),
		).toEqual({
			complete: false,
			total_available: 10,
			returned: 30,
	...
	it("reports COMPLETE when every upstream record was fetched despite child rows inflating total", () => {
		expect(
			workspaceCompleteness(10, { row_count: 250, primary_row_count: 10 }),
		).toEqual({
			complete: true,
		});
	});
